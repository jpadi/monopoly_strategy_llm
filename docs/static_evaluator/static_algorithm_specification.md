...
- Cash received is read from `transfer.amount` when `asset_type = cash`.
- Property identity is read from `transfer.asset_id` when `asset_type = property`.
- Property `amount` must equal `0` and is ignored after validation; it must never multiply printed property value.
- A property transfer with `amount = 0` is still a complete transfer of the property identified by `asset_id`.
...
The Published Trade Ratio is capped:

```
MAX_TRADE_RATIO = 5.0

Published Trade Ratio =
min(Raw Trade Ratio, MAX_TRADE_RATIO)
```

The cap prevents extreme ratios from disproportionately dominating classification when one player's Strategic Value is near zero.

The raw value must remain available in the evidence for auditability.

The Published Trade Ratio is the canonical result used for classification thresholds.
...
def calculate_trade_ratio(
    ...
    raw_ratio = highest / lowest
    published_ratio = min(raw_ratio, MAX_TRADE_RATIO)

    return published_ratio
```
...
- Required intermediate values: highest strategic value, lowest strategic value, division denominator, raw trade ratio, maximum trade ratio, published trade ratio.
- Result: number (`published_trade_ratio`).
